...
```
---
**if you have gradle installed on your system try gradle installDebug / gradle assembleDebug**
---

## 📦 Gradle Installation & Setup

To run build commands faster without downloading Gradle every time, you should install it manually on your system:

1. **Download:** Go to the [Gradle Services](https://services.gradle.org/distributions/) and download the **gradle-9.7.1-bin.zip** (or latest).
2. **Extract:** Unzip it to a permanent location (e.g., `C:\Gradle`).
3. **Set Path:** Add the `bin` folder to your system variables (see below).
...
### 🔹 Global (Permanent)
**Best for:** Your primary development machine. Once set, commands like `gradle` will work in *any* terminal window without setup.
...
4. Under **System variables**, click **New** to add:
   ...
   - `GRADLE_HOME`: `C:\Gradle\gradle-9.7.1` (Adjust to your extraction path)
