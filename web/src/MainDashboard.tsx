...
export default function MainDashboard() {
  ...
  const [missionToOpen, setMissionToOpen] = useState<number | null>(null);

  const openMissionFromReports = (missionId: number) => {
    setMissionToOpen(missionId);
  ...
  const [isConnectionLost /*, setIsConnectionLost */] =
            ...
            {panicMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-64 bg-white text-slate-800 rounded-lg shadow-xl border border-slate-200 overflow-hidden z-[100]">
                <div className="px-4 py-2.5 text-sm font-semibold text-slate-500">
                  {t.panic.activeAlerts}
                </div>

                {panicError ? (
                  <div className="px-4 pb-2.5 text-xs text-amber-600">
                    {panicError}
                  </div>
                ) : panics.length === 0 ? (
                  <div className="px-4 pb-2.5 text-xs text-slate-300">
                    {t.panic.noActiveAlerts}
                  </div>
                ) : (
                  panics.map((panic) => (
                    <div
                    ...
                    >
                      <span>
                        ...
                        <span className="text-xs text-slate-400 ml-2">
                          {t.panic.badge} {panic.officer.badge_number}
                        </span>
                      </span>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div dir="ltr">
              <button
                ...
                className="px-3 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-sm font-semibold text-white transition-colors cursor-pointer"
              >
                🌐 {language === 'en' ? 'AR' : 'EN'}
              </button>
            </div>
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden relative">
        {isConnectionLost ? (
          ...
          <>
            ...
            {activeTab === 'Reports' && <ReportsPage onMissionSelect={openMissionFromReports} />}
            {activeTab === 'Live map' && (
              <LiveMapPage focus={mapFocus} />
            )}
            {activeTab === 'Missions' && (
              <MissionsPage
                initialMissionId={missionToOpen}
                onInitialMissionOpened={() => setMissionToOpen(null)}
              />
            )}
          </>
        )}
